  ...
  class UeContextTransferReqData
    ...
    def self.openapi_types
      {
        ...
        :'access_type' => :'AccessType',
        :'plmn_id' => :'PlmnId',
        :'reg_request' => :'N1MessageContainer',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::UeContextTransferReqData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::UeContextTransferReqData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reg_request')
        self.reg_request = attributes[:'reg_request']
      end
    ...
    def hash
      [reason, access_type, plmn_id, reg_request, supported_features].hash
    end
