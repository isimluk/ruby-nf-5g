  ...
  class Guami
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError, 'The input argument (attributes) must be a hash in `Core5g::Guami` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `Core5g::Guami`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      self.amf_id = (attributes[:amf_id] if attributes.key?(:amf_id))
    end
    ...
    def list_invalid_properties
      ...
      invalid_properties.push('invalid value for "amf_id", amf_id cannot be nil.') if @amf_id.nil?

      pattern = /^[A-Fa-f0-9]{6}$/
      unless @amf_id&.match?(pattern)
        invalid_properties.push("invalid value for \"amf_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @amf_id.nil?
      return false unless /^[A-Fa-f0-9]{6}$/.match?(@amf_id)

      true
    end

    # Custom attribute writer method with validation
    # @param [Object] amf_id Value to be assigned
    def amf_id=(amf_id)
      raise ArgumentError, 'amf_id cannot be nil' if amf_id.nil?

      pattern = /^[A-Fa-f0-9]{6}$/
      unless amf_id&.match?(pattern)
        raise ArgumentError,
              "invalid value for \"amf_id\", must conform to the pattern #{pattern}."
      end
