...
module Core5g
  ...
  class TacRange
    attr_accessor :start, :_end, :pattern
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError, 'The input argument (attributes) must be a hash in `Core5g::TacRange` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `Core5g::TacRange`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      self._end = attributes[:_end] if attributes.key?(:_end)

      return unless attributes.key?(:pattern)
    ...
    def valid?
      ...
      return false if !@start.nil? && @start !~ /^([A-Fa-f0-9]{4}|[A-Fa-f0-9]{6})$/
      return false if !@_end.nil? && @_end !~ /^([A-Fa-f0-9]{4}|[A-Fa-f0-9]{6})$/
