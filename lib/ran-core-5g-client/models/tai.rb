  ...
  class Tai
    ...
    def initialize(attributes = {})
      unless attributes.is_a?(Hash)
        raise ArgumentError, 'The input argument (attributes) must be a hash in `Core5g::Tai` initialize method'
      end
      ...
      attributes = attributes.each_with_object({}) do |(k, v), h|
        unless self.class.attribute_map.key?(k.to_sym)
          raise ArgumentError,
                "`#{k}` is not a valid attribute in `Core5g::Tai`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
      ...
      self.tac = (attributes[:tac] if attributes.key?(:tac))
    ...
    def list_invalid_properties
      ...
      invalid_properties.push('invalid value for "tac", tac cannot be nil.') if @tac.nil?

      pattern = /(^[A-Fa-f0-9]{4}$)|(^[A-Fa-f0-9]{6}$)/
      unless @tac&.match?(pattern)
        invalid_properties.push("invalid value for \"tac\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @tac.nil?
      return false unless @tac&.match?(/(^[A-Fa-f0-9]{4}$)|(^[A-Fa-f0-9]{6}$)/)
    ...
    # @param [Object] tac Value to be assigned
    def tac=(tac)
      raise ArgumentError, 'tac cannot be nil' if tac.nil?

      pattern = /(^[A-Fa-f0-9]{4}$)|(^[A-Fa-f0-9]{6}$)/
      unless tac&.match?(pattern)
        raise ArgumentError, "invalid value for \"tac\", must conform to the pattern #{pattern}."
      end
